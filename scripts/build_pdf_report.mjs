...
/* ===== ESTRUCTURA EN 2 FILAS (evita que el mapa invada comunas) ===== */
.layout{
  display:grid;
  grid-template-rows: 168mm auto; /* fila superior fija */
  gap: 4mm;
}
...
/* MAPA controlado (no “flex:1” infinito) */
.mapCard{
  height: 168mm;
  display:flex;
  flex-direction:column;
...
.mapImg{
  width:100%;
  height:100%;
  object-fit: contain;      /* clave: no agranda/corta */
  object-position: 50% 65%; /* leve énfasis al sur sin inflar */
  display:block;
}
...
<body>
  ...
  <!-- FILA INFERIOR: comunas a todo ancho -->
